...
I used Microsoft Excel^2^ to record data in between each 2 trials (Excel Version 16.71). Prior to carrying out the experiment and collecting data, I randomized the order of the "wet" and "dry" treatments for the 100 total shot attempts; there were 50 allocated attempts per treatment. This then acted as the order in which I carried out each trial. Since the order of the treatments was pre-randomized, I recorded whether or not the ball made it into the cup, and if so, I also took note of the corresponding number of the cup the ball had landed into after every set of two trials. Each cup is assigned a unique number, from integers 1 to 10 (see Figure 1). If the ball did not make it into any cup for unsuccessful shots, I denoted that with the number 0 for the variable which stores the information about which cup the ball had landed into.

With the statistical software R and the tidyverse package, I used a one-sided 2 proportion z-test to conduct the analysis to compare the accuracy of the dunk method against the dry ball method (R version 4.3.0^3^, tidyverse v. 2.0.0; Wickham et al., 2023^4^).
...
Of the 100 total shot attempts, the ball successfully went into the cup 38% of the time, disregarding shot method (Figure 2). When considering shot method, using the dunk (wet ball) method resulted in a 19% shot accuracy, which was far from significant, using a 5% significance level (z-stat = 0, p = 0.5, n = 100). The 95% confidence interval for the mean difference proportions of success of each play method was (-0.16, 1.00). This interval includes that the value the mean difference in proportions is 0, which leads us to fail to reject the null hypothesis. Thus we cannot conclude that using the dunk method improves shot accuracy.
...
6.  Nichols, D. (n.d.). *Coloring for Colorblindness*. Davidmathlogic.com. Retrieved May 31, 2023, from https://davidmathlogic.com/colorblind/#%23005AB5-%23DC3220
7.  *Two-Proportions Z-Test in R*. Statistical Tools for High-Throughput Data Analysis. Retrieved May 31, 2023, from http://www.sthda.com/english/wiki/two-proportions-z-test-in-r#case-of-large-sample-sizes
...
[Link](https://github.com/hpawig/tech-report/tree/d8208e91da91bf4f5193ddcec597a3f16fcd2456/data/data_raw) to the location of the raw Excel data sheet on GitHub.
